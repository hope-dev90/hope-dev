          ...
          >
            ...
            <div className="flex-1 overflow-auto bg-navy/5 flex items-center justify-center p-4">
              {cert.type === "image" ? (
                ...
                <iframe src={cert.file} title={cert.name}
                  className="w-full rounded-xl shadow-lg bg-white" style={{ height: "65vh" }} />
              )}
            </div>
          </motion.div>
        </motion.div>
  ...
  return (
    <>
      <motion.div
        ...
        className="bg-white rounded-3xl shadow-card ring-1 ring-black/5 p-6 sm:p-7 h-full flex flex-col scroll-mt-24"
      >
        ...
        >
          {preview.map((cert, i) => (
            <motion.li key={cert.id} variants={fadeUp} custom={i * 0.06}>
              <motion.button
        ...
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
          <Link
            to="/cv"
            className="mt-2 inline-flex w-full items-center justify-center gap-2 border border-orange text-orange hover:bg-orange hover:text-white text-xs font-semibold rounded-full py-2.5 transition-colors"
          >
